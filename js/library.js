...
function flashChangeContainers() {
    ...
    const changeContainers = (el) => {
        ...
        const deactive = (i) => {
            items[i].classList.remove('fl-show');
    ...
    const isInView = () => {
        ...
        if (elementTop <= scrollLimit) {
            ...
            for (let i = 0; i < counters.length; i++) {
                flashCounter(counters[i], "get-from-target", 0, 3, 1)
            }
...
function flashFormCheck(form) {
    ...
    const resetAll = () => {
        ...
        for (let i = 0; i < messages.length; i++) {
            messages[i].classList.remove('fl-show');
        }

        for (let i = 0; i < len; i++) {
            fields[i].classList.remove('fl-form-required-error');
        }
